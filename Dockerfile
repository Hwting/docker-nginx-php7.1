...
RUN sed -i -e"s/worker_processes  1/worker_processes 2/" /etc/nginx/nginx.conf && \
	...
	sed -i -e "s/;cgi.fix_pathinfo=1/cgi.fix_pathinfo=0/g" /etc/php/7.1/fpm/php.ini && \
        sed -i -e "s/;opcache.memory_consumption=128/opcache.memory_consumption=128/g" /etc/php/7.1/fpm/php.ini && \
        sed -i -e "s/;opcache.interned_strings_buffer=8/opcache.interned_strings_buffer=8/g" /etc/php/7.1/fpm/php.ini && \
        sed -i -e "s/;opcache.max_accelerated_files=10000/opcache.max_accelerated_files=4000/g" /etc/php/7.1/fpm/php.ini && \
        sed -i -e "s/;opcache.revalidate_freq=2/opcache.revalidate_freq=60/g" /etc/php/7.1/fpm/php.ini && \
        sed -i -e "s/;opcache.fast_shutdown=0/opcache.fast_shutdown=1/g" /etc/php/7.1/fpm/php.ini && \
        sed -i -e "s/;opcache.enable_cli=0/opcache.enable_cli=1/g" /etc/php/7.1/fpm/php.ini && \
	...
	sed -i -e "s/pm.max_children = 5/pm.max_children = 5/g" /etc/php/7.1/fpm/pool.d/www.conf && \
	sed -i -e "s/pm.start_servers = 2/pm.start_servers = 2/g" /etc/php/7.1/fpm/pool.d/www.conf && \
	sed -i -e "s/pm.min_spare_servers = 1/pm.min_spare_servers = 1/g" /etc/php/7.1/fpm/pool.d/www.conf && \
